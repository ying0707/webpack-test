...
/* eslint-disable */
const Wallet = function (priv, pub, path, hwType, hwTransport, mnemonic) {
  const mnemonics = mnemonic || hd.bip39.generateMnemonic(128);
  const seed = hd.bip39.mnemonicToSeed(mnemonics);
  const hdWallet = hd.HDKey.fromMasterSeed(seed);
  const childkey = hdWallet.derive("m/44'/60'/0'/0/0");
  // if (typeof priv !== 'undefined') {
  //   // this.privKey = priv.length == 32 ? priv : Buffer(priv, 'hex');
  //   this.privKey = priv;
  // } else {
  //   this.privKey = childkey.privateKey;
  // }
  this.privKey = typeof priv !== 'undefined' ? priv : childkey.privateKey;
  this.pubKey = pub;
  this.path = path;
  this.hwType = hwType;
...
Wallet.mnemonic = function(mnemonic) {
  return new Wallet(undefined,undefined,undefined,undefined,undefined,mnemonic);
}
...
Wallet.prototype.setTokens = function () {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    const autoTokens = globalFuncs.localStorage.getItem('autoLoadTokens');
    const autoLoadTokens = autoTokens || [];
    const thisAddr = tokens[i].address;
  ...
  const conflictWithDefaultTokens = [];
  for (let e = 0; e < storedTokens.length; e++) {
...
function removeConflictingTokensFromLocalStorage(conflictLocalTokens, localTokens) {
  for (let i = 0; i < conflictLocalTokens.length; i++) {
    for (let e = 0; e < localTokens.length; e++) {
...
function removeAllTokenConflicts(conflictWithDefaultTokens, localTokens) {
  const deConflictedTokens = removeConflictingTokensFromLocalStorage(conflictWithDefaultTokens, localTokens);
  const deDuplicatedTokens = removeDuplicates(deConflictedTokens, 'symbol');
...
Wallet.prototype.setBalance = function (callback) {
  ...
  ajaxReq.getBalance(parentObj.getAddressString(), (data) => {
...
Wallet.fromEthSale = function (input, password) {
  ...
  const wallet = new Wallet(ethUtil.sha3(seed));
