...
globalFuncs.getParityMsg = function (str) {
  for (const reg in this.parityErrors) {
...
globalFuncs.getEthNodeMsg = function (str) {
  const ethNode = this.getEthNodeName();
...
globalFuncs.checkAndRedirectHTTPS = function () {
  const host = 'myetherwallet.com';
  const hostw = 'https://www.myetherwallet.com';
...
function getFromLS(key, errorMsg) {
  const localStorageItemString = globalFuncs.localStorage.getItem(key);
...
//   const defaultNodes = require('./nodes').nodeList;
...
//   const nodeErrMsg = 'Node does not exist, contact support@myetherwallet.com CODE:localstorageNodeMissing';
//   // localStorage selected node
//   const currentNodeKey = getFromLS('curNode', nodeErrMsg).key;
//   // custom nodes in local storage
//   const customLocalNodes = getFromLS('localNodes') || [];

//   const customNodeNetworkType = currentNodeKey.split('_')[1];

//   const isCustomNode = !!customLocalNodes.find((currentLocalCustomNode) => {
//     return currentLocalCustomNode.options === customNodeNetworkType;
//   });

//   let defaultNode;
//   let firstCustomNodeWithMatchingNetwork;
//   let defaultTokens;
...
//     firstCustomNodeWithMatchingNetwork = customLocalNodes.find((currentLocalCustomNode) => {
//       return currentLocalCustomNode.options === customNodeNetworkType;
//     });
...
function isDuplicateTokenAddress(tokenOne, tokenTwo, currentNetwork) {
  const hasNetwork = tokenTwo.network;
  if (hasNetwork) {
...
globalFuncs.doesTokenExistInDefaultTokens = function (token, defaultTokensAndNetworkType) {
  for (let i = 0; i < defaultTokensAndNetworkType.defaultTokens.length; i++) {
    const currentDefaultToken = defaultTokensAndNetworkType.defaultTokens[i];
    const isDuplicateToken = globalFuncs.isDuplicateToken(currentDefaultToken, token, defaultTokensAndNetworkType.networkType);
...
globalFuncs.saveTokenToLocal = function (localToken, callback) {
  try {
    if (!ethFuncs.validateEtherAddress(localToken.contractAdd)) { throw globalFuncs.errorMsgs[5]; } else if (!globalFuncs.isNumeric(localToken.decimals) || parseFloat(localToken.decimals) < 0) { throw globalFuncs.errorMsgs[7]; } else if (globalFuncs.isAlphaNumericOrSpec(localToken.symbol) || localToken.symbol === '') { throw globalFuncs.errorMsgs[19]; }
    const storedTokens = globalFuncs.localStorage.getItem('localTokens', null) != null ? JSON.parse(globalFuncs.localStorage.getItem('localTokens')) : [];
    ...
    const defaultTokensAndNetworkType = globalFuncs.getDefaultTokensAndNetworkType();
...
globalFuncs.removeTokenFromLocal = function (symbol, tokenObj) {
  const storedTokens = globalFuncs.localStorage.getItem('localTokens', null) != null ? JSON.parse(globalFuncs.localStorage.getItem('localTokens', null)) : [];
